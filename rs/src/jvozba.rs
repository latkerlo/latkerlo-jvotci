...
use crate::{
    ...
    rafsi::RAFSI,
    tarmi::{
        is_consonant, is_gismu, is_only_lojban_characters, is_valid_rafsi, is_vowel, rafsi_tarmi,
        tarmi_ignoring_hyphen, Tarmi,
    },
...
pub fn score(rafsi: &str) -> usize {
    1000 * rafsi.len() - 500 * rafsi.matches('\'').count() + 100 * rafsi.matches('y').count()
        - 10 * tarmi_ignoring_hyphen(rafsi).as_usize()
        - rafsi.matches(|c| "aeiou".contains(c)).count()
}
...
pub fn combine(
    ...
    if is_consonant(lujvo_final)
        && is_consonant(rafsi_initial)
        && !VALID.contains(&(lujvo_final.to_string() + &rafsi_initial.to_string()).as_str())
    {
        return None;
    }
    if ["ndj", "ndz", "ntc", "nts"]
        .contains(&(lujvo_final.to_string() + &slice(rafsi, 0, 2)).as_str())
    {
        return None;
    }
...
pub fn update_current_best(
    ...
    mut current_best: [BestLujvoMap; 2],
) -> [BestLujvoMap; 2] {
    if let Some((tosmabru, res_score, res_lujvo)) = result {
        let lujvo_final = char(&res_lujvo, res_lujvo.len() - 1).to_string();
        if !current_best[tosmabru].contains_key(&lujvo_final)
            || current_best[tosmabru].get(&lujvo_final).unwrap().1 > res_score
        {
