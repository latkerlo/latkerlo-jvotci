...
function processTanru(tanru) {
    ...
    let expandedValsiList = [];
    valsiList.forEach(valsi => {
        expandedValsiList = expandedValsiList.concat(!valsi.includes("-") ? splitWords(valsi) : [valsi]);
    });
    return expandedValsiList;
}
function getRafsiListList(valsiList, generateCmene = false) {
...
function combine(lujvo, rafsi, lujvo_score, isTosmabru, generateCmene, tanruLen = 0) {
    ...
    if (["ndj", "ndz", "ntc", "nts"].includes(lujvoFinal + rafsi.slice(0, 2))) {
        return null;
    }
...
function updateCurrentBest(result, currentBest) {
    if (result == null)
        return;
...
function getLujvo(tanru, generateCmene = false) {
    return getLujvo2(processTanru(tanru), generateCmene);
}
function getLujvo2(valsiList, generateCmene = false) {
    const rafsiListList = getRafsiListList(valsiList, generateCmene);
    let currentBest = [new Map(), new Map()];
    rafsiListList[0].forEach((rafsi0) => {
        rafsiListList[1].forEach((rafsi1) => {
            const isTosmabru = tarmiIgnoringHyphen(rafsi0) == Tarmi.CVC
                && rafsi0.slice(-1) !== "y"
                && !generateCmene ? 1 : 0;
            const result = combine(rafsi0, rafsi1, score(rafsi0), isTosmabru, generateCmene, rafsiListList.length);
        ...
        rafsiList.forEach((rafsi) => {
            for (let isTosmabru = 0; isTosmabru < 2; isTosmabru++) {
                previousBest[isTosmabru].forEach((lujvoAndScore, _) => {
                    const result = combine(lujvoAndScore[0], rafsi, lujvoAndScore[1], isTosmabru, generateCmene);
                    updateCurrentBest(result, currentBest);
                });
            }
        });
