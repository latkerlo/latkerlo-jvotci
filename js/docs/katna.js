 ...
 */
function searchSelrafsiFromRafsi(rafsi) {
 ...
 */
function compareLujvoPieces(corr, other) {
    let i = 0;
 ...
 * @returns List of lujvo pieces (rafsi and hyphens).
 */
function jvokaha2(lujvo, { yHyphens = YHyphenSetting.STANDARD, allowMZ = false } = {}) {
    ...
    while (true) {
        ...
        if (res.length > 0 && res[res.length - 1].length !== 1) { // hyphen cannot begin a word; nor can two hyphens
            ...
            else if (yHyphens !== YHyphenSetting.FORCE_Y
 ...
 */
function getVeljvo(lujvo, { yHyphens = YHyphenSetting.STANDARD, expRafsiShapes = false, consonants = ConsonantSetting.CLUSTER, glides = false, allowMZ = false } = {}) {
    const [bType, rafsiList] = analyseBrivla(lujvo, {
        yHyphens: yHyphens,
