...
function isGismuOrLujvo(aString: string): boolean {
  if (isGismu(aString))
    return true;
...
function splitOneCmavo(aString: string): string[] {
  let i = 0;
  let willEnd = false;
  while (i < aString.length) {
    if (
      ["ai", "ei", "oi", "au"].includes(aString.slice(i, i + 2)) 
      && i + 2 < aString.length 
...
function splitWords(aString: string): string[] {
  if (aString.length === 0)
    return [];
  ...
  const consonantCluster = firstFive.search(
